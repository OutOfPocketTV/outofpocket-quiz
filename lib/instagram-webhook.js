...
async function handleComment({ value, accountId, store, token, log }) {
  ...
  try {
    if (keyword) {
      const done = await answerKeyword({ api: ig, token, comment, cfg, progress });
      log(`instagram-webhook: ${done.action} "${comment.text.slice(0, 60)}" -> "${done.reply}"` + (done.error ? ` (${done.error})` : ''));
      return done.action;
    }
    ...
    // Only hand the comment back for a retry if nothing was sent. Once a DM or
    // reply has been attempted it may have gone through even if Instagram said
    // it failed (it does that), so the claim stays and no retry sends twice.
    if (!progress.attempted) await store.release(comment.id).catch(() => {});
    throw err;
  }
}
